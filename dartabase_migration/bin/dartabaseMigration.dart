...
Future serverStatus(String rootPath) {
  Completer completer = new Completer();
  ...
  if (DBCore.adapter == DBCore.PGSQL) {
    ...
    try {
      ConnectionPool pool;
      if (DBCore.ssl) {
        pool = new ConnectionPool(host: DBCore.host, port: DBCore.port, user: DBCore.username, password: DBCore.password, db: DBCore.database, max: 1, useSSL: true);
      } else {
