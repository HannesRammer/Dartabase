...
===================
    ...
    *see http://pub.dartlang.org/packages/dartabase_model
	...
	Version
		0.3.0 breaking change -> added autogenerated created_at and updated_at, see "COLUMN CREATED/UPDATED" info below
		0.2.0 breaking change -> added autogenerated id, see "COLUMN ID" info below 
...
---------------------------------
	...
	just download the new version of dartabase migration 
	and replace all files from your current running version
	it is important to keep 'dartabase_migration/bin/projectsMapping.json' 
	but since this file will be autogenerated only on initiation of dartabase migration there should not be a problem
	
	LOST 'projectsMapping.json' 
		DONT PAN!C 
		  simply create a file named projectsMapping.json inside 'dartabase_migration/bin/'
	...
	MOVED/RENAMED project
		  Just rename the values when moving/renaming one of your projects
		  
		  now you should be able to find your projects again when running dbUp.dart or dbDown.dart
...
---------
	...
	The 'id' column will be autogenerated by 'Dartabase Migration' for every table as primary key, 
	so dont add 'id' in any of the migration files.
	This is to pass 'Dartabase Model' the work of deciding when to create or update an Object * 
	*see http://pub.dartlang.org/packages/dartabase_model   	
	
	V>=0.2.0
    if you had already created a migration file with a comuln 'id' in version < 0.2.0 
    simply remove the entry for 'id' from all your migration file UP and DOWN keys 
